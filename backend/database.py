    ...
    async def create_incident(self, incident: Incident) -> Incident:
        await self.conn.execute(
            ...
            (
                ...
                incident.ai_assessment.model_dump_json() if incident.ai_assessment else None,
    ...
    async def create_responder(self, responder: Responder) -> Responder:
        await self.conn.execute(
            ...
            (
                ...
                responder.team,
                responder.phone,
                responder.current_location.model_dump_json() if responder.current_location else None,
    ...
    async def create_triage_entry(self, entry: TriageEntry) -> TriageEntry:
        await self.conn.execute(
            ...
            (
                ...
                entry.ai_notes,
                entry.responder_id,
                entry.location.model_dump_json() if entry.location else None,
            )
        )
        await self.conn.commit()
        return entry
    ...
    async def create_alert(self, alert: Alert) -> Alert:
        await self.conn.execute(
            ...
            (
                ...
                alert.coordinates.model_dump_json() if alert.coordinates else None,
                alert.radius_km,
