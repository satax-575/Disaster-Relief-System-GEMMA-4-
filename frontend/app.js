    ...
    <div class="stat-card" style="padding: 1.5rem; display: flex; flex-direction: column; gap: 1rem;">
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; background: rgba(0,0,0,0.2); padding: 0.75rem; border-radius: 8px; border: 1px solid rgba(255,255,255,0.05);">
        ...
        <div>
          ...
          <div style="font-size: 0.85rem; color: #e2e8f0; font-family: 'JetBrains Mono', monospace;">${r.current_location ? `${r.current_location.lat.toFixed(4)}, ${r.current_location.lng.toFixed(4)}` : 'GPS Offline'}</div>
        </div>
      </div>
  ...
  const sendMessage = () => {
    ...
    if (chatWs && chatWs.readyState === WebSocket.OPEN) {
      ...
      fetch(`${API_BASE}/chat`, {
        ...
        let text_resp = data.message || data.content || 'No response received.';
        text_resp = text_resp
          .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
          .replace(/\n/g, '<br>');
        ...
        messages.insertAdjacentHTML('beforeend', `
          <div class="msg ai">
            <div class="msg-bubble">${text_resp}</div>
            <div class="msg-meta">${data.model_used || 'raksha-ai'} • ${new Date().toLocaleTimeString()}</div>
          </div>
        `);
        messages.scrollTop = messages.scrollHeight;
      }).catch(err => {
        ...
        messages.insertAdjacentHTML('beforeend', `
          <div class="msg ai">
            <div class="msg-bubble">⚠️ Connection error. Please check your network and try again.</div>
          </div>
        `);
